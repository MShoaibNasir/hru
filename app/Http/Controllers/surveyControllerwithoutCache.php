...
if ($beneficiary_details) {
    ...
    // Caching the answer check
        $answer_check = Cache::remember("answer_check_{$id}", 300, function () use ($id) {
            //return Answer::where('survey_form_id', $id)->select('id')->first();
    ...
    $comment_missing_document = CommentMissingDocument::where('survey_id', $id)->select('id','created_role','comment')->first();
