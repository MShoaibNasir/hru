...
if ($request->has('pending_days') && $request->get('pending_days') != null) {
    $pending_days = $request->get('pending_days');

    // Define the day ranges for each pending_days value
    $dateRanges = [
    ...
    if (isset($dateRanges[$pending_days])) {
        ...
        $surveydata->where('m_status', 'P')->where(function ($query) use ($startDate, $endDate) {
            $query->whereNotNull('m_last_action_date');

            if ($startDate && $endDate) {
                $query->whereBetween('m_last_action_date', [$startDate, $endDate]);
            } elseif ($startDate) {
                $query->where('m_last_action_date', '>=', $startDate);
            } else {
                $query->where('m_last_action_date', '<=', $endDate);
            }

            $query->orWhere(function ($subQuery) use ($startDate, $endDate) {
    ...
    if ($userRole == 1) {
        if (in_array($status, ['P', 'H'])) {
            ...
            $surveydata->whereHas('getformstatusold', function ($q) use ($request, $department_new, $updateByMapping) {
                if ($request->has('department') && isset($updateByMapping[$department_new])) {
                    $q->where('update_by', $updateByMapping[$department_new]);
                }
                $q->where('form_status', 'A');
            });
        } elseif ($status == 'R') {
            $surveydata->where('m_last_action_role_id', $department_new)
                       ->where('m_last_action', $status);
        }
    } else {
        if ($status == 'P') {
            if ($userRole == 51) {
                ...
                $upper_rejected_ids = SurveyData::where('m_last_action_role_id', $upper_role->id)
                                                ->where('m_last_action', 'R')
                                                ->pluck('id');
                $surveydata->where('m_status', $status)
                           ->whereNotIn('id', $upper_rejected_ids);
            }
        } elseif ($status == 'H') {
            $surveydata->where('m_status', $status);
        } elseif ($status == 'A') {
            ...
            if ($userRole == $department_new) {
                $surveydata->where('m_last_action_role_id', $userRole)
                           ->where('m_last_action_user_id', Auth::user()->id)
                           ->where('m_last_action', $status);
            } else {
                $surveydata->where('m_last_action_role_id', $department_new)
                           ->where('m_last_action', $status);
            ...
            $surveydata->where('m_last_action_role_id', $status)
                       ->where('m_last_action', 'R');
        }
    }
}
	...
	public function finance_trail_history(Request $request)
    {
		if($request->ajax()){
		    ...
		    $surveydata = SurveyData::where('id', $survey_id)->first();
            //dump($surveydata->getfinancetrail);
			return view('dashboard.report.render_finance_trail_history', compact('surveydata'))->render(); 
		}
	}
