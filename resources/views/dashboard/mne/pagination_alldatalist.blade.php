...
@foreach($chunks as $item)
  ...
  @endphp
                            <tr style='background-color: {{$item->priority==1 ? '#19875433' : 'transparent'}}';>
                                <td><a href='{{ route("mne.view",[$item->id]) }}' target="_blank" class='btn btn-success'>View ID:{{ $item->id }}</a></td>
                                ...
                                <a class="btn btn-sm btn-danger report_trail_btn" style="height:30px;" mne_id="{{ $item->id }}" href="javascript:void(0)">Report Trail</a>
                                @else
                                No Trail
                                @endif
                                </td>
