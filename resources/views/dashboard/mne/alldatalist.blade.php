            ...
            <div class="col-md-12">
                                  ...
                                  <!--Toolbar-->
                                    	<div class="toolbar">
                                            ...
                                            <div class="filters-toolbar__item mb-3 col-md-3">
                                                ...
                                                {!! Form::select('department', ['27' => 'Validator', '34' => 'IP', '37' => 'PSIA', '51' => 'Monitoring and Evaluation (HRU)'], null, array('placeholder' => 'Select Department', 'class' => 'department form-control select2', 'id'=>'department')) !!}
                                            </div>
                ...
                <div class="row">										
                									...
                									<div class="filters-toolbar__item mb-3 col-md-4">
                                                      ...
                                                      {{ Form::select('sorting',[ 'id' => 'MNE ID', 'ref_no' => 'Refrence Number'],'id',['class'=>'form-control', 'id'=>'sorting']) }}
                                                    </div>
...
<x-backend.mne.reporttrail_modal />
@endsection
@push('ayiscss')
...
$(document).ready(function(){	
    ...
    function filter_data(currentpage)
    {
       ...
       $.ajax({
          type: 'POST',
          data:{action:action, stage:stage, action_condition:action_condition, department:department, lot_id:lot_id, district_id:district_id, tehsil_id:tehsil_id, uc_id:uc_id, b_reference_number:b_reference_number, beneficiary_name:beneficiary_name, cnic:cnic, sorting:sorting, direction:direction, qty:qty, ayis_page:ayis_page, _token: '{{csrf_token()}}'},
          url: "{{ route('total_mne_datalist_fetch_data') }}",
    ...
    $('body').on('change', '#sorting, #direction, #qty, #stage, #action_condition, #department, #lot_id, #district_id, #tehsil_id, #uc_id', function(e){
            e.preventDefault();
            filter_data();
    });
    ...
    $('body').on('click', '.report_trail_btn', function(e){
        ...
        $.ajax({
              url: "{{ route('mne_status_trail_history') }}",
              type: 'GET',
              data:{_token: '{{csrf_token()}}', mne_id:mne_id},
              //dataType: 'JSON',
