    ...
    <div class="container-fluid pt-4 px-4 form_width">
        <div class="bg-light text-center rounded p-4">
            <div class="d-flex align-items-center justify-content-between mb-4">
                <h6 class="mb-0">Gender Safeguard Report</h6>
            </div>
            ...
            <div class="col-md-12">
                                  ...
                                  <!--Toolbar-->
                                     ...
                                     {{--       
                                            ...
                                            <div class="filters-toolbar__item mb-3 col-md-3">
                                                ...
                                                {!! Form::select('department', ['34' => 'IP', '61' => 'Gender Specialist'], null, array('placeholder' => 'Select Department', 'class' => 'department form-control select2', 'id'=>'department')) !!}
                                            </div>
                                            <div class="filters-toolbar__item mb-3 col-md-3">
                                                <label class="form-label">{{ __('Status') }}</label>
                                                {!! Form::select('item_status', ['P' => 'Pending Cases', 'CR' => 'Register Cases','C'=>'Close Cases'], null, array('placeholder' => 'Select Case Status', 'class' => 'item_status form-control select2', 'id'=>'item_status')) !!}
                                            </div>
                							
                										
                										
                										<div class="filters-toolbar__item mb-3 col-md-4">
                									        <label for="Search">Search By VRC Name</label>
                                                       {{ Form::text('vrc',null,['class'=>'form-control', 'id'=>'vrc','placeholder'=>'VRC Name']) }}
                										</div>
                										{{--
                ...
                <div class="row">										
                									...
                									<div class="filters-toolbar__item mb-3 col-md-4">
                                                      <label for="Sorting">Form Name</label>
                                                      {{ Form::select('form_name',[ '43' => 'Gender Safeguards Screening checklist','46'=>'Gender Screening Checklist (Mitigation Measures)'],null,['class'=>'form-control', 'id'=>'form_name']) }}
                                                    </div>
...
$(document).ready(function(){	
    ...
    function filter_data(currentpage)
    {
       ...
       $.ajax({
          type: 'POST',
          data:{action:action, stage:stage, action_condition:action_condition, department:department,item_status:item_status, lot_id:lot_id, district_id:district_id, tehsil_id:tehsil_id, uc_id:uc_id, b_reference_number:b_reference_number, beneficiary_name:beneficiary_name, vrc:vrc, sorting:sorting, direction:direction,form_name:form_name, qty:qty, ayis_page:ayis_page, _token: '{{csrf_token()}}'},
          url: "{{ route('total_gender_datalist_fetch_data') }}",
    ...
    $("#b_reference_number, #beneficiary_name, #vrc").on('keyup keydown', function() {
    ...
    $('body').on('change', '#sorting, #direction, #qty, #stage,#form_name, #action_condition, #department,#item_status, #lot_id, #district_id, #tehsil_id, #uc_id', function(e){
            e.preventDefault();
            filter_data();
    });
    ...
    $('body').on('click', '.report_trail_btn', function(e){
        ...
        $.ajax({
              url: "{{ route('gender_status_trail_history') }}",
              type: 'GET',
