...
<script src="{{asset('dashboard\js\survey_list.js')}}"></script>
  ...
  @if(session('error'))
    ...
    function redirectLocation(){
        if(current_user==40){
        window.location.href = 'https://mis.hru.org.pk/survey/ceo/pending/list';
        }
        else if(current_user==48){
            window.location.reload();
        }
        else if(current_user==51){
            window.location.href = 'https://mis.hru.org.pk/report/survey/datalist';
        }
        else{
