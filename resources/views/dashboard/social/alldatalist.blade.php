    ...
    <div class="container-fluid pt-4 px-4 form_width">
        <div class="bg-light text-center rounded p-4">
            <div class="d-flex align-items-center justify-content-between mb-4">
                <h6 class="mb-0">Social Safeguard Report</h6>
            </div>
                                  ...
                                  <!--Toolbar-->
                                    	<div class="toolbar">
                                            ...
                                            <div class="filters-toolbar__item mb-3 col-md-3">
                                                ...
                                                {!! Form::select('form_name', ['45' => 'Social Safeguards Screening Checklist (Mitigation Measures)', '44' => 'Social Safeguards Screening Checklist'], null, array('placeholder' => 'Select Form', 'class' => 'form_name form-control select2', 'id'=>'form_name')) !!}
                                            </div>  
                                            
                                            <div class="filters-toolbar__item mb-3 col-md-3">
                                                <label class="form-label">{{ __('Department') }}</label>
                                                {!! Form::select('department', ['34' => 'IP', '63' => 'Social Specialist'], null, array('placeholder' => 'Select Department', 'class' => 'department form-control select2', 'id'=>'department')) !!}
                                            </div>
                                            <div class="filters-toolbar__item mb-3 col-md-3">
                                                <label class="form-label">{{ __('Select Status') }}</label>
                                                {!! Form::select('status', ['P' => 'Pending Cases', 'A' => 'Approved Cases','R'=>'Reject Cases','CR'=>'Register Cases','C'=>'Close Cases'], null, array('placeholder' => 'Select Status', 'class' => 'department form-control select2', 'id'=>'status')) !!}
                                            </div>
...
<x-backend.social.reporttrail_modal />
@endsection
@push('ayiscss')
...
$(document).ready(function(){	
    ...
    function filter_data(currentpage)
    {
       ...
       $.ajax({
          type: 'POST',
          data:{action:action, stage:stage, form_name:form_name, department:department,status:status, lot_id:lot_id, district_id:district_id, tehsil_id:tehsil_id, uc_id:uc_id, b_reference_number:b_reference_number, vrc:vrc, cnic:cnic, sorting:sorting, direction:direction, qty:qty, ayis_page:ayis_page, _token: '{{csrf_token()}}'},
          url: "{{ route('total_social_datalist_fetch_data') }}",
    ...
    $("#b_reference_number, #vrc, #cnic").on('keyup keydown', function() {
		filter_data();
    });
    
    

    $('body').on('change', '#sorting, #direction, #qty, #stage, #form_name, #department, #lot_id, #district_id, #tehsil_id, #uc_id , #status', function(e){
            e.preventDefault();
            filter_data();
    });
    ...
    $('body').on('click', '.report_trail_btn', function(e){
        ...
        $.ajax({
              url: "{{ route('social_status_trail_history') }}",
              type: 'GET',
