    ...
    <div class="container-fluid pt-4 px-4 form_width">
        <div class="bg-light text-center rounded p-4">
            <div class="d-flex align-items-center justify-content-between mb-4">
                <h6 class="mb-0">Ready For Disbursement List</h6>
            </div>
            ...
            <div class="col-md-12">
                                    	...
                                    	<div class="toolbar">
                                            ...
                                            <div class="filters-toolbar__item mb-3 col-md-4">
                                            	...
                                            	<div class="row">
                										...
                										{{ Form::text('single_bank_name',null,['class'=>'form-control', 'id'=>'single_bank_name','placeholder'=>'Bank Name']) }}
                										</div>
                ...
                <div class="row">										
                                    ...
                                    <form method='post' action='{{route("get_export_ready_for_disbursment")}}'>
                                        @csrf
                                    ...
                                    <input type='hidden' class='ref_no_data' name='ref_no_data'>
                                    <input type='submit' value='Generate Sheet' id="generate_sheet" class='btn btn-success'>
                                    </form>
                                    <form method='post' action='{{route("moveToFirstTrenchBulk")}}'>
                                        @csrf
                                   
                                    <input type='hidden' class='ref_no_data' name='ref_no_data'>
                                    <input type='submit' value='Move To Tranche' id="generate_sheet" class='btn btn-success'>
                                    </form>
                                    </div>
...
$(document).ready(function(){	
    ...
    function filter_data(currentpage)
    {
        ...
        var single_bank_name = $("#single_bank_name").val();
		var cnic = $("#cnic").val();
       ...
       $.ajax({
          type: 'POST',
          data:{action:action, district:district, tehsil_id:tehsil_id, uc_id:uc_id, b_reference_number:b_reference_number,single_bank_name:single_bank_name,bank_name:bank_name,beneficiary_name:beneficiary_name, cnic:cnic, sorting:sorting, direction:direction, qty:qty, ayis_page:ayis_page, _token: '{{csrf_token()}}'},
          url: "{{ route('beneficiaryDisbursmentFetchData') }}",
    ...
    $("#b_reference_number, #beneficiary_name, #cnic, #single_bank_name").on('keyup keydown', function() {
		filter_data();
    });
    
    

    $('body').on('change', '#sorting, #direction, #qty, #district, #tehsil_id, #uc_id,#bank_name', function(e){
            e.preventDefault();
            filter_data();
    });
