...
@php
    $lastPart = last(request()->segments());
@endphp
        ...
        <div class="bg-light text-center rounded p-4">
            ...
            <div class="col-md-12">
                                    	...
                                    	<div class="toolbar">
                                            ...
                                            <div class="filters-toolbar__item mb-3 col-md-4">
                                              ...
                                              {{-- <div class="filters-toolbar__item mb-3 col-md-4">
                                                      ...
                                                      {{ Form::select('trench',['1' => 'First Tranche', '2' => 'Second Tranche','3' => 'Third Tranche'],$data->trench_no,['class'=>'form-control', 'id'=>'trench']) }}
                                                    </div>  --}}
                ...
                <div class="row">										
                									...
                									<div class="filters-toolbar__item mb-3 col-md-4">
                                                      <label for="Quantity">Selected Data</label>
                                                      {{ Form::select('qty', [''=>'Select Data',1=>'Selected Data',0=>'Un Selected Data'], null,['class'=>'form-control', 'id'=>'checked_data']) }}
                                                    </div>
                                        ...
                                        <!--End Toolbar-->
                                    <!--form start-->
                                    <form method='post' action='{{route("update_first_batch",[$data->id])}}'>
                                        @csrf
                                        <div class='row'>
                                            <h4>Edit Batch Data</h4>
                                        </div>
                                        <div class='row'>
                                            <div class='col-6'>
                                                <label>Batch No</label>
                                                <input type='text' name='batch_no' required value='{{$data->batch_no}}'  class='form-control'>
                                            </div>
                                            <div class='col-6'>
                                                <label>Cheque No</label>
                                                <input type='text' name='cheque_no' value='{{$data->cheque_no}}' class='form-control'>
                                            </div>
                                            <div class='col-6'>
                                                <label>Date</label>
                                                <input type='date' name='batch_created_date' value='{{$data->batch_created_date}}' required class='form-control'>
                                            </div>
                                            <div class='col-6'>
                                                <label>Select Bank</label>
                                                <select class='form-control' name='bank_id' required>
                                                    @foreach($bank as $key=>$item)
                                                    <option value='{{$key}}' {{$key==$data->bank_id ? 'selected' : ''}}>{{$item}}</option>
                                                    @endforeach
                                                </select>
                                            </div>
                                            <input type='hidden' name='ref_no' value='{{$edit_ref_no}}' id='ref_no_data' />
                                            <input type='hidden' name='trench_no' value='{{$data->trench_no}}' id='trench_no' />
                                            <input type='hidden' value='{{$lastPart}}' id='lastPart' >
                                            <input type='submit' value='Submit'  class='form-control hover-btn btn btn-success my-4' style='width:200px; color:black;' >
                                        </div>
                                    </form>
                                    <h5>{{$trench_no}} Tranche </h5>
...
$(document).ready(function(){	
    ...
    function filter_data(currentpage)
    {
        ...
        var checked_data = $("#checked_data").val();
        var qty = $("#qty").val();
        var trench = $("#trench").val();
        //var trench = $("#trench_no").val();
        //alert(trench);
       ...
       $.ajax({
          type: 'POST',
          data:{action:action, district:district,lastPart:lastPart,tehsil_id:tehsil_id, uc_id:uc_id, trench:trench, b_reference_number:b_reference_number,bank_name:bank_name,beneficiary_name:beneficiary_name, cnic:cnic, sorting:sorting, direction:direction,checked_data:checked_data, qty:qty, ayis_page:ayis_page, _token: '{{csrf_token()}}'},
          url: "{{ route('firsttrench_fetch_data_for_edit') }}",
    ...
    $('body').on('change', '#sorting, #direction,#checked_data, #qty, #district, #tehsil_id, #uc_id, #trench', function(e){
            e.preventDefault();
            filter_data();
    });
    ...
    var edit_ref_no = @json($edit_ref_no);
  

    if (typeof edit_ref_no === "string") {
        edit_ref_no = edit_ref_no.split(",");
    }

    var selectedValues = edit_ref_no.length > 0 ? edit_ref_no : [];
    console.log(selectedValues);
    
    let count = Math.max(0, selectedValues.length);
    ...
    $(document).on("change", ".change_id_status", function () {
        var value = $(this).val();
        var token = $('meta[name="csrf-token"]').attr("content");
        $.ajax({
            type: "POST",
            url: `{{ route('remove_check_box_value') }}`,
            data: {
                value: value,
                _token: token,
            },
